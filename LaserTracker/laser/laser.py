...
class Console:
    ...
    def set_image(self, image):
        # Check if image is None to avoid AttributeError
        if image is None:
            # Create a black error image with error text
            h, w = self.size[1], self.size[0]  # size is (width, height)
            self.image = np.zeros((h, w, 3), dtype=np.uint8)
            cv2.putText(self.image, "Camera Error", (10, h//2), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(self.image, "Check camera connection", (10, h//2 + 40), 
...
def openCam(camera_id, cam_api=cv2.CAP_DSHOW):
    cam = cv2.VideoCapture(camera_id, cam_api)
    if not cam.isOpened():
        print ("Could not open Camera #" + str(camera_id))
        print ("Trying another one...")
        cam = cv2.VideoCapture(-1, cam_api)
        if not cam.isOpened():
            if cam_api == cv2.CAP_ANY:
                print ("All webcam failed. Quitting.")
                exit()
            else:
                return openCam(camera_id, cam_api=cv2.CAP_ANY)
    
    # Validate that the camera can actually capture frames
    width, height = camSize(cam)
    print ("Cam opened with size = " + str(width) + ", " + str(height))
    
    # If dimensions are 0, try to read a test frame to validate the camera
    if width == 0 or height == 0:
        print ("Camera dimensions invalid, testing frame capture...")
        ret, test_frame = cam.read()
        if not ret or test_frame is None:
            print ("Camera failed validation test, trying fallback...")
            cam.release()
            if cam_api != cv2.CAP_ANY:
                return openCam(camera_id, cam_api=cv2.CAP_ANY)
            else:
                print ("All camera methods failed. Quitting.")
                exit()
        else:
            # Update dimensions from actual frame
            width, height = test_frame.shape[1], test_frame.shape[0]
            print ("Camera validated with actual size = " + str(width) + ", " + str(height))
...
def readCam(cam):
    ...
    if not retval:
        ...
        # Create a simple error image instead of trying to load missing file
        img = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(img, "Camera Error", (200, 240), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    return (img)
